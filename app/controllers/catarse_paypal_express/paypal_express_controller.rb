...
class CatarsePaypalExpress::PaypalExpressController < ApplicationController
  ...
  def process_paypal_message(data)
    backer.payment_notifications.create!({
      extra_data: data
    })
  ...
  def setup_gateway
    if PaymentEngines.configuration[:paypal_username] and ::Configuration[:paypal_password] and ::Configuration[:paypal_signature]
